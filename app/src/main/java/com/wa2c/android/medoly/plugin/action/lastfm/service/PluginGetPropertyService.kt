...
class PluginGetPropertyService : AbstractPluginService(PluginGetPropertyService::class.java.simpleName) {
     ...
     * Get properties.
     */
    private suspend fun getProperties(): CommandResult {
...
//            }
                val track = Track.getInfo(
                    artistText,
                    trackText,
                    Token.getConsumerKey()
                ) // TODO: Error occurred with session
...
//                resultExtra[getString(R.string.label_extra_data_user_play_count)] = track.userPlaycount.toString()
                if (track.playcount > 0)
                    resultExtra[getString(R.string.label_extra_data_play_count)] =
                        track.playcount.toString()
                if (track.listeners > 0)
                    resultExtra[getString(R.string.label_extra_data_listener_count)] =
                        track.listeners.toString()
                resultExtra[getString(R.string.label_extra_data_lastfm_track_url)] = track.url
                return@withContext  CommandResult.SUCCEEDED
            } catch (e: Exception) {
                logE(e)
                resultProperty = null
                resultExtra = null
